...
class InternalJsonParserTest {
    ...
    fun testNumbers() {
        ...
        for (testCase in testCases) {
            ...
            assertEquals(testCase.toDouble(), n.toDouble(), Double.MIN_NORMAL)
            val n2 = JsonParser.any().from(testCase.toUpperCase()) as Number
            assertEquals(testCase.toUpperCase().toDouble(), n2.toDouble(), Double.MIN_NORMAL)
        }
    }
    ...
    fun testNegativeZero() {
        assertEquals("-0.0", (JsonParser.any().from("-0") as Number).toDouble().toString())
        assertEquals("-0.0", (JsonParser.any().from("-0.0") as Number).toDouble().toString())
        assertEquals("-0.0", (JsonParser.any().from("-0.0e0") as Number).toDouble().toString())
        assertEquals("-0.0", (JsonParser.any().from("-0e0") as Number).toDouble().toString())
        assertEquals("-0.0", (JsonParser.any().from("-0e1") as Number).toDouble().toString())
        assertEquals("-0.0", (JsonParser.any().from("-0e-1") as Number).toDouble().toString())
        assertEquals("-0.0", (JsonParser.any().from("-0e-0") as Number).toDouble().toString())
        assertEquals("-0.0", (JsonParser.any().from("-0e-01") as Number).toDouble().toString())
        assertEquals("-0.0", (JsonParser.any().from("-0e-000000000001") as Number).toDouble().toString())
...
//    }
    ...
    private fun testException(e: JsonParserException, linePos: Int, charPos: Int, inError: String) {
        assertEquals("line $linePos char $charPos",
                "line " + e.linePosition + " char " + e.charPosition)
        assertTrue(e.message!!.contains(inError), "Error did not contain '" + inError + "': " + e.message)
    }
}
