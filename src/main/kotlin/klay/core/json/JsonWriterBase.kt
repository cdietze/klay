...
internal open class JsonWriterBase<SELF : JsonSink<SELF>>(protected val appendable: Appendable) : JsonSink<SELF> {
    ...
    override fun `object`(obj: Map<*, *>): SELF {
        ...
        for ((key1, o) in obj) {
            if (key1 !is String)
                throw JsonWriterException("Invalid key type for obj: " + (key1?.let { it::class } ?: "null"))
            value(key1, o)
        }
        return end()
    }
    ...
    override fun value(o: Any?): SELF {
        if (o == null)
            ...
            throw JsonWriterException("Unable to handle type: " + o::class)// TODO(mmastrac): Implement in future playn-server
    ...
    override fun end(): SELF {
        ...
        first = false
        inObject = states.removeAt(states.size - 1)
