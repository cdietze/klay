...
internal object Crossing {
    ...
    fun solveCubic(eqn: DoubleArray, res: DoubleArray): Int {
        ...
        val Q = (a * a - 3f * b) / 9f
        val R = (2.0 * a * a * a - 9.0 * a * b + 27f * c) / 54f
        val Q3 = Q * Q * Q
        val R2 = R * R
        val n = -a / 3f

        if (R2 < Q3) {
            val t = Math.acos(R / Math.sqrt(Q3)) / 3f
            val p = 2f * Math.PI / 3f
            val m = -2f * Math.sqrt(Q)
            res[rc++] = m * Math.cos(t) + n
            res[rc++] = m * Math.cos(t + p) + n
            res[rc++] = m * Math.cos(t - p) + n
        } else {
            // Debug.println("R2 >= Q3 (" + R2 + "/" + Q3 + ")");
            var A = Math.pow(Math.abs(R) + Math.sqrt(R2 - Q3), (1f / 3f).toDouble())
            if (R > 0f) {
                A = -A
            }
            // if (A == 0f) {
     ...
     */
    protected fun fixRoots(res: DoubleArray, rc: Int): Int {
        var tc = 0
        for (i in 0..rc - 1) {
            out@ run {
                for (j in i + 1..rc - 1) {
    ...
    class QuadCurve(x1: Double, y1: Double, cx: Double, cy: Double, x2: Double, y2: Double) {
        ...
        fun cross(res: DoubleArray, rc: Int, py1: Double, py2: Double): Int {
            var cross = 0
            ...
            for (i in 0..rc - 1) {
                ...
                // CURVE-START
                if (t < DELTA) {
                    if (py1 < 0f && (if (bx != 0.0) bx else ax - bx) < 0f) {
                        cross--
                    }
                    continue
                }
        ...
        fun solvePoint(res: DoubleArray, px: Double): Int {
            val eqn = doubleArrayOf(-px, Bx, Ax)
            return solveQuad(eqn, res)
        }

        fun solveExtreme(res: DoubleArray): Int {
            var rc = 0
            if (Ax != 0.0) {
                res[rc++] = -Bx / (Ax + Ax)
            }
        ...
        fun addBound(bound: DoubleArray, bc: Int, res: DoubleArray, rc: Int, minX: Double, maxX: Double,
    ...
    class CubicCurveH(x1: Double, y1: Double, cx1: Double, cy1: Double, cx2: Double, cy2: Double,
                      x2: Double, y2: Double) {
        internal var ax: Double = 0.toDouble()
        internal var ay: Double = 0.toDouble()
        internal var bx: Double = 0.toDouble()
        ...
        fun cross(res: DoubleArray, rc: Int, py1: Double, py2: Double): Int {
            ...
            for (i in 0..rc - 1) {
                ...
                // CURVE-START
                if (t < DELTA) {
                    if (py1 < 0f && (if (bx != 0.0) bx else if (cx != bx) cx - bx else ax - cx) < 0f) {
                        cross--
                    }
                    continue
                }
        ...
        fun solveExtremeX(res: DoubleArray): Int {
            val eqn = doubleArrayOf(Cx, Bx2, Ax3)
            return solveQuad(eqn, res)
        }

        fun solveExtremeY(res: DoubleArray): Int {
     ...
     */
    fun crossQuad(x1: Double, y1: Double, cx: Double, cy: Double, x2: Double, y2: Double,
     ...
     */
    fun crossCubic(x1: Double, y1: Double, cx1: Double, cy1: Double, cx2: Double,
                   cy2: Double, x2: Double, y2: Double, x: Double, y: Double): Int {
        // LEFT/RIGHT/UP/EMPTY
     ...
     */
    fun crossShape(s: IShape, x: Double, y: Double): Int {
     ...
     */
    fun intersectLine(x1: Double, y1: Double, x2: Double, y2: Double, rx1: Double,
                      ry1: Double, rx2: Double, ry2: Double): Int {
        // LEFT/RIGHT/UP
     ...
     * intersect
     */
    fun intersectQuad(x1: Double, y1: Double, cx: Double, cy: Double, x2: Double,
                      y2: Double, rx1: Double, ry1: Double, rx2: Double, ry2: Double): Int {
        // LEFT/RIGHT/UP ------------------------------------------------------
     ...
     * intersect
     */
    fun intersectCubic(x1: Double, y1: Double, cx1: Double, cy1: Double,
                       cx2: Double, cy2: Double, x2: Double, y2: Double,
                       rx1: Double, ry1: Double, rx2: Double, ry2: Double): Int {
        // LEFT/RIGHT/UP
     ...
     */
    fun intersectShape(s: IShape, x: Double, y: Double, w: Double, h: Double): Int {
     ...
     * Sorts a bound array.
     */
    protected fun sortBound(bound: DoubleArray, bc: Int) {
        var i = 0
        while (i < bc - 4) {
            var k = i
            var j = i + 4
     ...
     */
    protected fun crossBound(bound: DoubleArray, bc: Int, py1: Double, py2: Double): Int {
        // LEFT/RIGHT
        if (bc == 0) {
            return 0
        }
