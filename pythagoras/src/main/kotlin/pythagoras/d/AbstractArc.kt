...
abstract class AbstractArc : RectangularShape(), IArc {
    ...
    fun startPoint(target: Point): Point {
        val a = Math.toRadians(angleStart())
    ...
    fun endPoint(target: Point): Point {
        val a = Math.toRadians(angleStart() + angleExtent())
        return target.set(x() + (1f + Math.cos(a)) * width() / 2f,
                y() + (1f - Math.sin(a)) * height() / 2f)
    }
    ...
    fun contains(px: Double, py: Double): Boolean {
        ...
        val containsAngle = containsAngle(Math.toDegrees(-Math.atan2(ny, nx)))
        if (arcType() == IArc.PIE) {
    ...
    fun bounds(target: Rectangle): Rectangle {
        ...
        var bx1 = if (containsAngle(180.0)) rx1 else Math.min(p1.x(), p2.x())
        var by1 = if (containsAngle(90.0)) ry1 else Math.min(p1.y(), p2.y())
        var bx2 = if (containsAngle(0.0)) rx2 else Math.max(p1.x(), p2.x())
        var by2 = if (containsAngle(270.0)) ry2 else Math.max(p1.y(), p2.y())
    ...
    protected fun normAngle(angle: Double): Double {
    ...
    protected class Iterator internal constructor(a: IArc,
        ...
        private var step: Double = 0.toDouble()
