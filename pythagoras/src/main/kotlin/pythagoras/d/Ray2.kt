...
class Ray2 : IRay2 {
    ...
    fun getIntersection(start: IVector, end: IVector, result: Vector): Boolean {
        ...
        if (Math.abs(divisor) < MathUtil.EPSILON) {
            ...
            val isect = t != java.lang.Float.MAX_VALUE.toDouble()
            if (isect) {
    ...
    fun getIntersection(center: IVector, radius: Double, result: Vector): Boolean {
        ...
        val t = (-b - Math.sqrt(radicand)) * 0.5f
        val isect = t >= 0f
        if (isect) {
            origin.addScaled(direction, t, result)
        }
     ...
     */
    protected fun getIntersection(pt: IVector): Double {
        if (Math.abs(direction.x) > Math.abs(direction.y)) {
            val t = (pt.x() - origin.x) / direction.x
            return if (t >= 0f && origin.y + t * direction.y == pt.y()) t else java.lang.Double.MAX_VALUE
        } else {
            val t = (pt.y() - origin.y) / direction.y
            return if (t >= 0f && origin.x + t * direction.x == pt.x()) t else java.lang.Double.MAX_VALUE
        }
    }
}
