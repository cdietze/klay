...
class Area : IShape, Cloneable {
    ...
    constructor(s: IShape) {
        val segmentCoords = DoubleArray(6)
        var lastMoveX = 0.0
        var lastMoveY = 0.0
        var rulesIndex = 0
        var coordsIndex = 0
    ...
    private fun addCurvePolygon(area: Area) {
        ...
        val resultCoords = DoubleArray(_coordsSize + area._coordsSize + intersectPoints.size)
    ...
    private fun subtractPolygon(area: Area) {
        ...
        do {
            ...
            if (curIndex < 0 || area.isVertex(_coords[2 * curIndex], _coords[2 * curIndex + 1]) &&
                    crossHelper.containsPoint(doubleArrayOf(_coords[2 * curIndex], _coords[2 * curIndex + 1])) &&
    ...
    private fun includeCoordsAndRules(
            ...
            resultCoords: DoubleArray, coords: DoubleArray, resultRulesPos: Int, resultCoordPos: Int,
            point: IntersectPoint, isCurrentArea: Boolean, way: Boolean, operation: Int): Int {
        var length = length
        var resultRulesPos = resultRulesPos
        var way = way

        val temp = DoubleArray(8 * length)
        var coordsCount = 0
        ...
        for (i in offset..length - 1) {
            ...
            when (rules[i]) {
                ...
                PathIterator.SEG_QUADTO -> {
                    ...
                    var coefs = doubleArrayOf(coords[index - 2], coords[index - 1], coords[index], coords[index + 1], coords[index + 2], coords[index + 3])
                ...
                PathIterator.SEG_CUBICTO -> {
                    ...
                    coefs = doubleArrayOf(coords[index - 2], coords[index - 1], coords[index], coords[index + 1], coords[index + 2], coords[index + 3], coords[index + 4], coords[index + 5])
                    isLeft = CrossingHelper.compare(
                            coords[index - 2], coords[index - 1], point.x(), point.y()) > 0
    ...
    private fun containsExact(x: Double, y: Double): Int {
        ...
        val segmentCoords = DoubleArray(6)
        val resultPoints = DoubleArray(6)
        var rule: Int
        var curX = -1.0
        var curY = -1.0
        var moveX = -1.0
    ...
    private fun reverseCopy(coords: DoubleArray) {
        val temp = DoubleArray(coords.size)
    ...
    private fun areaBoundsSquare(): Double {
        val bounds = bounds()
        return bounds.height() * bounds.width()
    }

    private fun isVertex(x: Double, y: Double): Boolean {
        var i = 0
        while (i < _coordsSize) {
    ...
    private fun adjustSize(array: DoubleArray, newSize: Int): DoubleArray {
        if (newSize <= array.size) {
            return array
        }
        val newArray = DoubleArray(2 * newSize)
    ...
    private inner class AreaPathIterator internal constructor(private val transform: Transform?) : PathIterator {
        ...
        override fun currentSegment(c: DoubleArray): Int {
            if (isDone) {
    ...
    private var _coords = DoubleArray(20)
