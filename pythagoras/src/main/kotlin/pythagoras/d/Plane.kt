...
class Plane : IPlane, Serializable {
    ...
    /** The plane constant.  */
    var constant: Double = 0.toDouble()

    /**
     * Creates a plane from the specified normal and constant.
     */
    constructor(normal: IVector3, constant: Double) {
        set(normal, constant)
    }
     ...
     * @return a reference to this plane (for chaining).
     */
    operator fun set(normal: IVector3, constant: Double): Plane {
     ...
     * @return a reference to this plane (for chaining).
     */
    fun set(values: DoubleArray): Plane {
     ...
     * @return a reference to this plane (for chaining).
     */
    operator fun set(a: Double, b: Double, c: Double, d: Double): Plane {
        _normal.set(a, b, c)
        constant = d
        return this
    }
    ...
    fun intersection(ray: IRay3, result: Vector3): Boolean {
        ...
        if (java.lang.Double.isNaN(distance) || distance < 0f) {
            return false
        } else {
    ...
    fun distance(ray: IRay3): Double {
        ...
        if (Math.abs(dividend) < MathUtil.EPSILON) {
            return 0.0 // origin is on plane
        } else if (Math.abs(divisor) < MathUtil.EPSILON) {
            return java.lang.Float.NaN.toDouble() // ray is parallel to plane
        } else {
    ...
    companion object {
        private const val serialVersionUID = 5309702666731799041L

        /** The X/Y plane.  */
        val XY_PLANE = Plane(Vector3.UNIT_Z, 0.0)

        /** The X/Z plane.  */
        val XZ_PLANE = Plane(Vector3.UNIT_Y, 0.0)

        /** The Y/Z plane.  */
        val YZ_PLANE = Plane(Vector3.UNIT_X, 0.0)
    }
}
