...
class InternalJsonNumberTest {
    ...
    @Throws(JsonParserException::class)
    fun testBigIntRead() {
        val array = JsonParser.array().from("[-30000000000000000000,30000000000000000000]")
        assertEquals(BigInteger::class, array[0]!!::class)
        assertEquals(BigInteger("-30000000000000000000"), array[0])
        assertEquals(BigInteger::class, array[1]!!::class)
        assertEquals(BigInteger("30000000000000000000"), array[1])
    }

    @Test
    fun testBigIntWrite() {
        val array = JsonArray.from(BigInteger.ONE, BigInteger("-30000000000000000000"), BigInteger(
                "30000000000000000000"))
        assertEquals("[1,-30000000000000000000,30000000000000000000]", JsonStringWriter().array(array).write())
    }
    ...
    @Throws(JsonParserException::class)
    fun testAroundEdges() {
        val array = JsonArray.from(Integer.MAX_VALUE, Integer.MAX_VALUE.toLong() + 1, Integer.MIN_VALUE,
                Integer.MIN_VALUE.toLong() - 1, java.lang.Long.MAX_VALUE, BigInteger.valueOf(java.lang.Long.MAX_VALUE).add(BigInteger.ONE),
                java.lang.Long.MIN_VALUE, BigInteger.valueOf(java.lang.Long.MIN_VALUE).subtract(BigInteger.ONE))
