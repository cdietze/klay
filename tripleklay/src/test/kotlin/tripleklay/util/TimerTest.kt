...
class TimerTest {
    ...
    fun testRepeat() {
        var time = 25.0
        val timer = Timer(plat::time, time)
        val r1 = Counter()
    ...
    @Test
    fun testInternalCancel() {
        val ran1 = InternalCanceler()
        val t = Timer(plat::time, 0.0)
        ran1.handle = t.every(2, ran1)
        for (ii in 0..6) t.update(plat, (ii + 1).toDouble())
        assertEquals(1, ran1.ranCount.toLong())
    }
