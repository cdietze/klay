...
open class Hud(protected val _game: SceneGame) {
    ...
    class Stock(game: SceneGame) : Hud(game) {
        ...
        protected fun willUpdate() {
            ...
            // int frames = maxOf(stats.frames, 1);
        ...
        _rows.add(object : Row {
            ...
            override fun update() {
                ...
                _size.setSize(llayout.size.width + GAP + _vlayout!!.size.width,
                        maxOf(llayout.size.height, _vlayout!!.size.height))
            }
    ...
    fun update() {
        ...
        for (row in _rows) {
            row.update()
            width = maxOf(row.size().width, width)
            labelWidth = maxOf(row.labelWidth(), labelWidth)
