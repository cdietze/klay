...
(override val layout: Layout) : Container.Mutable<T>() {
    ...
    fun add(vararg children: Element<*>): T {
        ...
        _children.addAll(children.asList())
        for (child in children) {
            didAdd(child)
        }
        invalidate()
    ...
    override fun childCount(): Int = _children.size

    override fun childAt(index: Int): Element<*> = _children[index]

    override fun iterator(): Iterator<Element<*>> = _children.iterator()
