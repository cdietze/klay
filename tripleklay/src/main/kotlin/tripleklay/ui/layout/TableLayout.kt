    ...
    override fun layout(elems: Container<*>,
        ...
        for (elem in elems) {
            ...
            for (ii in 0..colspan - 1) {
                colWidth += maxOf(0f, m.columnWidths!![col + ii] + if (freeWeight == 0f) 0f else freeExtra * _columns[col + ii]._weight)
            }
            ...
            if (colWidth > 0 && elem.isVisible) {
                ...
                val elemWidth = if (colspan > 1 || ccfg._stretch)
                    ...
                    minOf(psize.width, colWidth)
                val elemHeight = if (_vstretch) rowHeight else minOf(psize.height, rowHeight)
    ...
    private fun computeMetrics(elems: Container<*>, hintX: Float, hintY: Float): Metrics {
        ...
        for (elem in elems) {
            ...
            if (elem.isVisible && _columns[col]._weight == 0f) {
                ...
                metrics.rowHeights!![row] = maxOf(metrics.rowHeights!![row], psize.height)
                ...
                if (colspan(elem) == 1) {
                    metrics.columnWidths!![col] = maxOf(metrics.columnWidths!![col], psize.width)
                }
            }
            ii += colspan(elem)
        }
