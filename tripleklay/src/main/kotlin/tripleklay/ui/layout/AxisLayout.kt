...
abstract class AxisLayout : Layout() {
    ...
    class Vertical : AxisLayout() {
        ...
        override fun layout(elems: Container<*>,
            ...
            val stretchHeight = maxOf(0f, height - m.gaps(_gap.toFloat()) - m.fixHeight)
    ...
    class Horizontal : AxisLayout() {
        ...
        override fun layout(elems: Container<*>,
            ...
            val stretchWidth = maxOf(0f, width - m.gaps(_gap.toFloat()) - m.fixWidth)
    ...
    protected fun computeMetrics(elems: Container<*>, hintX: Float, hintY: Float,
        ...
        for (elem in elems) {
            ...
            if (!c.stretch) {
                ...
                m.maxWidth = maxOf(m.maxWidth, pwidth)
                m.maxHeight = maxOf(m.maxHeight, pheight)
                m.fixWidth += pwidth
                m.fixHeight += pheight
            } else {
                m.stretchers++
        ...
        for (elem in elems) {
            ...
            val pwidth = psize.width
            val pheight = psize.height
            m.unitWidth = maxOf(m.unitWidth, pwidth / c.weight)
            m.unitHeight = maxOf(m.unitHeight, pheight / c.weight)
            m.maxWidth = maxOf(m.maxWidth, pwidth)
            m.maxHeight = maxOf(m.maxHeight, pheight)
        }
