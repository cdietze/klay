...
(
    ...
    class Constraint(private val _pos: Position, val _stretch: Boolean) : Layout.Constraint() {
        ...
        fun adjust(pref: IDimension, boundary: Rectangle): Dimension {
            ...
            dim.width = minOf(dim.width, boundary.width)
            dim.height = minOf(dim.height, boundary.height)
            return dim
        }
     ...
     */
    constructor(gaps: Float = 0f) : this(gaps, gaps)
    ...
    private inner class Slots internal constructor(elems: Container<*>) {
        ...
        internal fun computeSize(hintX: Float, hintY: Float): Dimension {
            ...
            for (pos in NS) {
                ...
                nsSize.width = maxOf(nsSize.width, dim.width)
                if (wce > 0) {
                    nsSize.height += vgap
                }
            }

            val ehintY = maxOf(0f, hintY - nsSize.height)
            val weSize = Dimension()
            for (pos in WE) {
                val dim = size(pos, 0f, ehintY) ?: continue
                weSize.width += dim.width
                weSize.height = maxOf(weSize.height, dim.height)
            }

            weSize.width += maxOf(wce - 1, 0) * hgap
            val ehintX = maxOf(0f, hintX - weSize.width)
            ...
            return Dimension(
                    maxOf(weSize.width, nsSize.width),
                    maxOf(weSize.height, nsSize.height))
        }
