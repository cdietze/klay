...
abstract class Animation protected constructor() {
    ...
    class One(private val _target: Value) : Interped<One>() {
        ...
        override fun toString(): String {
            return this::class.simpleName + " start:" + _start + " to " + _to
        }

        private var _from = Float.MIN_VALUE
    ...
    class Two(private val _value: XYValue) : Interped<Two>() {
        ...
        private var _fromx = Float.MIN_VALUE
        private var _fromy = Float.MIN_VALUE
    ...
    fun completeChain() {
        ...
        val seen = mutableListOf<Animation>()
        var anim: Animation? = this
        while (anim != null && !seen.any { it === anim }) {
            seen.add(anim)
            check(!anim._canceled, { "Cannot complete() a canceled animation." })
            anim.makeComplete()
            anim.cancel()
            anim = anim.next()
        }
    ...
    protected abstract inner class ChainBuilder : AnimBuilder() {
        override fun <T : Animation> add(anim: T): T {
            check(!_added, { "Cannot add multiple animations off the same then()" })
            _added = true
