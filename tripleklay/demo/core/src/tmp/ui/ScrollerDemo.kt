...
import tripleklay.ui.Scroller
import tripleklay.ui.Scroller.Behavior
import tripleklay.ui.Shim
import tripleklay.ui.SizableWidget
...
class ScrollerDemo : DemoScreen() {
    ...
    protected fun createIface(root: Root): Group {
        ...
        val updateSize = object : UnitSlot() {
            fun onEmit() {
        ...
        val beh = Button(Behavior.BOTH.name).onClick(object : Slot<Button>() {
            fun onEmit(source: Button) {
        ...
        scroll.contentClicked().connect(object : Slot<Pointer.Event>() {
            fun onEmit(e: Pointer.Event) {
